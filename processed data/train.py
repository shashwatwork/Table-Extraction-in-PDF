...
def train(lr=0.01,epochs=1000,load=False):
    ...
    if load:
        model.load_state_dict(torch.load("model/TableUnetlossCrossed.pth"))
    allloss=[0]
    ...
    for i in range(epochs):
        ...
        if difference<0.0:
            print("saving ........")
            torch.save(model.state_dict(), "./model/TableUnetlossCrossed256.pth")
            high_loss_value+=1
        ...
        if high_loss_value==2 and lr_counter<=len(lossChanges):
            ...
            torch.save(model.state_dict(), "./model/TableUnetEnd256.pth")
            break
        if i%10==0:
            print("***************** Saving Model   ****************")
            torch.save(model.state_dict(), "./model/TableUnet256.pth")
